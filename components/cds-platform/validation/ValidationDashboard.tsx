...
export default function ValidationDashboard() {
  ...
  const loadFairnessMetrics = useCallback(async () => {
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900">
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="mt-8">
          ...
          <TabsContent value="subgroup" className="mt-6 focus-visible:outline-none">
            {report ? (
              <SubgroupPanel report={report} />
            ) : (
              <EmptyState loading={loading} />
            )}
          </TabsContent>

          <TabsContent value="regulatory" className="mt-6 focus-visible:outline-none">
            <RegulatoryTab
              subgroupAnalysis={report?.subgroupAnalysis ?? []}
              overallAuc={report?.summary.aucRoc ?? null}
              onLoadFairness={loadFairnessMetrics}
              fairnessLoading={loading}
            />
          </TabsContent>
        </Tabs>
      </div>
...
function EmptyState({ loading }: { loading: boolean }) {
  if (loading) {
    return <p className="text-slate-500">Running validation…</p>;
  }
  return <p className="text-slate-500">Run a validation to see results.</p>;
}
...
function PerformancePanel({
  ...
  report: ValidationReportState;
  rocChartData: { fpr: number; tpr: number }[];
  calibrationChartData: { predicted: number; actual: number }[];
}) {
  return (
    <>
      <section>
        ...
        <div className="mt-3 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
            ...
            <div className="mt-1 flex items-center gap-2">
              <span className="text-xl font-semibold">{report.summary.aucRoc.toFixed(3)}</span>
              <Badge
                pass={report.summary.aucRoc >= AUC_TARGET}
                label={report.summary.aucRoc >= AUC_TARGET ? 'Pass' : 'Below target'}
              />
            </div>
          ...
          <div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
            ...
            <div className="mt-1 flex items-center gap-2">
              ...
              <Badge
                pass={report.summary.accuracy3Class >= 0.7}
                label={report.summary.accuracy3Class >= 0.7 ? 'Pass' : 'Below target'}
              />
            </div>
          </div>
          <div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
          ...
          {calibrationChartData.length > 0 ? (
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={calibrationChartData} margin={{ top: 5, right: 20, left: 0, bottom: 5 }}>
                ...
                <Tooltip />
                <Legend />
                <Line type="monotone" dataKey="actual" stroke="#0f766e" strokeWidth={2} name="Actual" dot />
                <Line
                  type="monotone"
                  dataKey="predicted"
                  stroke="#94a3b8"
      ...
      <div className="mt-6 text-xs text-slate-500">
        Report generated at {report.generatedAt} · Model: {report.modelVersion} · Dataset:{' '}
...
function SubgroupPanel({ report }: { report: ValidationReportState }) {
  if (!report.subgroupAnalysis?.length) {
    return (
      <p className="text-sm text-slate-500">
        No subgroup data. Re-run validation with &quot;Include subgroups&quot; enabled.
      </p>
    );
  }
  ...
  return (
    <section>
      ...
      <p className="mt-1 text-sm text-slate-600">
        Overall AUC-ROC: {report.summary.aucRoc.toFixed(3)} · Generated {report.generatedAt}
      </p>
            ...
            {report.subgroupAnalysis.map((sg) => (
              <tr key={sg.key}>
                ...
                <td className="px-4 py-2 text-right">
                  {(sg.metrics.accuracy3Class * 100).toFixed(1)}%
                </td>
                <td className="px-4 py-2 text-right">
                  {(sg.metrics.sensitivity * 100).toFixed(1)}%
                </td>
                <td className="px-4 py-2 text-right">
                  {(sg.metrics.specificity * 100).toFixed(1)}%
                </td>
              </tr>
            ))}
          </tbody>
...
function ConfusionMatrixHeatmap({
  ...
  return (
    <div className="inline-block">
      <div
        className="grid gap-px rounded-lg border border-slate-200 bg-slate-200 p-px"
