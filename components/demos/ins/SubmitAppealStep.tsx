  ...
  if (processing.isGenerating) {
    return (
      <div className="space-y-6">
        <h2 className="font-heading text-2xl font-semibold text-arka-text">Submit / Appeal Generator</h2>
        <Card variant="default">
          <CardContent className="p-8">
            <div className="flex flex-col items-center gap-4">
              <FileText className="h-12 w-12 text-arka-deep animate-pulse" />
              <p className="text-arka-text-muted font-medium">{processing.processingMessage ?? "Generating appeal..."}</p>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="font-heading text-2xl font-semibold text-arka-text">Submit / Appeal Generator</h2>
          <p className="text-arka-text-soft text-sm">Submit prior auth or generate appeal letter.</p>
        </div>
        ...
        <Card variant="default">
          ...
          <CardContent className="p-6">
            <div className="prose prose-invert prose-sm max-w-none">
              <p className="text-arka-text-muted whitespace-pre-wrap">{generatedAppeal.letterContent}</p>
            </div>
            {generatedAppeal.citedGuidelines?.length > 0 && (
              <p className="text-xs text-arka-text-soft mt-4">Cited: {generatedAppeal.citedGuidelines.join("; ")}</p>
            )}
          </CardContent>
        </Card>
      ) : (
